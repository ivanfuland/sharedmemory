...
RUBRIC_PROMPT = (
    ...
    "示例：输入：[idx=0 user] 老兰定 LFT 前6月只模拟盘，资金5-10万封顶 → "
...
_DEDUP_SYS="你是去重器。合并语义同点 atom（M1 同点不同措辞 / M2 同一关系双向各记），保留信息最全规范措辞；独立事实不合并。只输出 JSON：{\"atoms\":[{\"entity\":\"..\",\"fact\":\"..\"}]}。"
_ALIGN_SYS="你是对齐器。统计列表 A、B 中语义指同一事实的配对数。只输出 JSON：{\"matched\": <int>}。"
_DUP_SYS="你是重复审计器。在 atom 列表中找语义指同一事实的重复对（dedup 漏并）。只输出 JSON：{\"dups\":[[i,j],...]}，i<j；无则 []。"
