...
日志正则原样摘自 everalgo.agent_memory.case（拿来主义：不重写、不"改进"上游的判据文本，
逐字节对齐真实产出）：
...
_SEMANTIC_PATTERN = re.compile(r"filtered out by LLM: ?(?P<reason>.*)")
# ⚠ 这四条正则是 plan 撰写时对 everalgo.agent_memory.case 真实日志文本的转录（含 U+2014
# em dash），不是从源码里机械提取的。实现时已重新对照当时的 `case.py` 逐字核对一遍
# （wheel cache: everalgo/agent_memory/case.py 第 106/116/124/604 行，含 U+2014 em dash
# 字节核实）——上游措辞可能已漂移，需在 Phase B 真跑前再核一遍。这不是唯一防线：
# `probe_calibrate`（§3 三个已知结局合成会话）是运行时的 backstop，正则测漏/测错会在
# 那一步被三选一的错误分类当场抓到，而不是喂到真样本才发现。
...
def classify_log_window(log_text: str) -> str:
    ...
    结构优先于语义：先扫 3 条结构门正则，命中即返回 STRUCTURAL_REJECT，不再看语义门。
    ...
    if _SEMANTIC_PATTERN.search(log_text):
